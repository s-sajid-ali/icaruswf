...
namespace icaruswf {
    ...
    public:
      ...
      LoadbalancingInput(fhicl::ParameterSet const& p,
          ...
          art::SourceHelper const& pm) : pm_(pm), 
                                         ...
                                         queue_{datastore_.openQueue<EID>(inputProcessname_, hepnos::QueueAccessMode::CONSUMER)}, 
  ...
  bool
    ...
    // how do we know we are done? 
      EID eid;
      bool got_new_event = queue_.pop(eid); // need to add a check here
      if (!got_new_event) return false;
      auto r = eid.run;
      auto sr = eid.subrun;
      auto ev = eid.event;
      art::Timestamp ts;
      ...
      auto event = subrun.createEvent(ev);
