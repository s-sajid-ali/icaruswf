...
read_product(hepnos::Event &event, std::string const &module_label,
             std::string const &instancename, std::string const &processname,
             bool strict) {
...
template <typename T>
art::ProductID create_productID(std::vector<T> const &t,
                                std::string const &module_label,
                                std::string const &instance_name = "") {
  auto const product_name = art::canonicalProductName(
      art::TypeID{t}.friendlyClassName(), module_label, instance_name,
...
read_assns(hepnos::Event &event, art::EventPrincipal *&outE,
           std::vector<A> const &a_col, std::vector<B> const &b_col,
...
bool read_daq(hepnos::Event &event, bool strict, art::EventPrincipal *&outE) {
  ...
  for (auto p : prodIds) {
    ...
    if (type == "std::vector<raw::RawDigit, std::allocator<raw::RawDigit> >") {
      auto prod = hepnos::read_product<raw::RawDigit>(event, label, instance,
                                                      process, strict);
...
bool read_wires(hepnos::Event &event, bool strict, art::EventPrincipal *&outE) {
  ...
  art::put_product_in_principal(std::move(wires0), *outE, "roifinder",
                                "PHYSCRATEDATATPCEE");
  art::put_product_in_principal(std::move(wires1), *outE, "roifinder",
                                "PHYSCRATEDATATPCEW");
  art::put_product_in_principal(std::move(wires2), *outE, "roifinder",
                                "PHYSCRATEDATATPCWE");
  art::put_product_in_principal(std::move(wires3), *outE, "roifinder",
                                "PHYSCRATEDATATPCWW");
  return true;
}

bool read_hits(hepnos::Event &event, bool strict, art::EventPrincipal *&outE) {
  auto hits0 = hepnos::read_product<recob::Hit>(event, "gaushitTPCEE", "",
                                                "HitFinding", strict);
  auto hits1 = hepnos::read_product<recob::Hit>(event, "gaushitTPCEW", "",
                                                "HitFinding", strict);
  auto hits2 = hepnos::read_product<recob::Hit>(event, "gaushitTPCWE", "",
                                                "HitFinding", strict);
  auto hits3 = hepnos::read_product<recob::Hit>(event, "gaushitTPCWW", "",
                                                "HitFinding", strict);
...
bool read_assns_hits_wires(hepnos::Event &event, bool strict,
...
public:
  template <typename T>
  void reconstitutes(std::string const &moduleLabel,
                     std::string const &instanceName,
                     std::string const &processName) {
    art::BranchDescription const bd{
        art::InEvent, moduleLabel,
        processName,  art::TypeID{typeid(T)}.className(),
        instanceName, {},
  ...
  HepnosInputSource(fhicl::ParameterSet const &p,
                    art::ProductRegistryHelper &rh, art::SourceHelper const &pm)
      : pm_(pm), datastore_{art::ServiceHandle<HepnosDataStore>()->getStore()},
    ...
    if (inputProcessname_ == "HitFinding") {
      ...
      rh.reconstitutes<std::vector<recob::Hit>, art::InEvent>("gaushitTPCWW",
                                                              "");
      rh.reconstitutes<art::Assns<recob::Hit, recob::Wire, void>, art::InEvent>(
          "gaushitTPCEE", "");
      rh.reconstitutes<art::Assns<recob::Hit, recob::Wire, void>, art::InEvent>(
          "gaushitTPCEW", "");
      rh.reconstitutes<art::Assns<recob::Hit, recob::Wire, void>, art::InEvent>(
  ...
  void run_hepnos_func(std::function<void(void)> work) {
  ...
  void readFile(std::string const &dsname, art::FileBlock *&fb) {
    ...
    fb = new art::FileBlock{art::FileFormatVersion{1, "HepnosSource"}, dsname};
    {
      std::function<void(void)> f = [&]() {
        es_ = dataset_.events();
        ev_ = es_->begin();
    ...
    size_t i = 0;
    // both estart and nskip shouldn't be specified together, its either n and
    // nskip Or n and estart
    ...
    if (firstEvent_ != -1u) {
      while (ev_->number() != firstEvent_ &&
             ...
             ev_->subrun().run().number() != firstRun_)
        ++ev_;
    }
  }
...
bool HepnosInputSource::readNext(art::RunPrincipal *const &inR,
...
template <> struct Source_wantFileServices<icaruswf::HepnosInputSource> {
