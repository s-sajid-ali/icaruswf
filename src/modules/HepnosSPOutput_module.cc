...
template <typename P>
P const *prodWithType(art::EDProduct const *product,
...
template <typename A, typename B>
void storeassns(hepnos::DataStore datastore, hepnos::Event &h_e,
                std::map<art::ProductID, hepnos::ProductID> const &a_map,
                art::InputTag const &a_t, art::Assns<A, B> const &a_assns) {
  hepnos::Assns<A, B> h_assns;
  h_assns.reserve(a_assns.size());

  for (auto const &[a, b] : a_assns) {
    //  std::cout <<"storeassns, art prod keys: " << a.key() << ", " << b.key()
    //  << std::endl; std::cout << "storeassns, art prod IDs: " << a.id() << ",
    //  " << b.id() << std::endl;
  ...
  // std::cout << "Assns in art event: " << a_t.process() << ", " << a_t.label()
  // << ", " << a_t.encode() << "\n";
}
...
void storeassns(hepnos::DataStore datastore, hepnos::Event &h_e,
                std::map<art::ProductID, hepnos::ProductID> const &a_map,
                art::InputTag const &a_t, art::Assns<A, B, D> const &a_assns) {
...
bool assnsWithType(art::EDProduct const *product,
                   art::BranchDescription const &pd, hepnos::DataStore ds,
                   hepnos::Event &h_e,
                   std::map<art::ProductID, hepnos::ProductID> const &a_map) {
...
public:
  ...
  update_map_hevent(hepnos::Event h_e, std::string current_processname) {
    ...
    for (auto p : prodIds) {
      p.unpackInformation(&datasetId, &run, &subrun, &hepnosevent, &hepnostag,
                          &type);
      auto [label, instance, processname] = splitTag(hepnostag);
      auto t = art::uniform_type_name(type);
      auto const product_name =
          art::canonicalProductName(art::friendlyname::friendlyName(t), label,
                                    instance, current_processname);
  ...
  update_map_aevent(EventPrincipal &a_e, hepnos::Event h_e,
                    hepnos::WriteBatch &batch, bool wantForwardProducts,
    ...
    for (auto const &pr : a_e) {
      ...
      if (any_of(begin(dropCommands), end(dropCommands),
                 [&pd](auto const &cmd) {
    ...
    spdlog::info("store times: num={}, avg={}, max={}, min={}",
                 stats.raw_storage_time.num, stats.raw_storage_time.avg,
                 stats.raw_storage_time.max, stats.raw_storage_time.min);

    spdlog::info("serialization times: num={}, avg={}, max={}, min={}",
                 stats.serialization_time.num, stats.serialization_time.avg,
                 stats.serialization_time.max, stats.serialization_time.min);

    return translator;
  }

  void store_aassns(art::EventPrincipal &p, hepnos::DataStore ds,
                    hepnos::Event h_e,
                    std::map<art::ProductID, hepnos::ProductID> &translator,
                    std::vector<std::regex> const &dropCommands) {
    ...
    for (auto const &pr : p) {
      ...
      // dynamic cast to the type we care about is needed here
      EDProduct const *product = oh.isValid() ? oh.wrapper() : nullptr;
      if (assnsWithType<raw::RawDigit, recob::Wire>(product, pd, ds, h_e,
                                                    translator)) {
        continue;
      }
      if (assnsWithType<raw::RawDigit, recob::Hit>(product, pd, ds, h_e,
                                                   translator)) {
        continue;
      }
      if (assnsWithType<recob::Wire, recob::Hit>(product, pd, ds, h_e,
                                                 translator)) {
        continue;
      }
      if (assnsWithType<recob::Seed, recob::Hit>(product, pd, ds, h_e,
                                                 translator)) {
        continue;
      }
      if (assnsWithType<recob::Cluster, recob::Hit>(product, pd, ds, h_e,
                                                    translator)) {
        continue;
      }
      if (assnsWithType<recob::Hit, recob::SpacePoint>(product, pd, ds, h_e,
                                                       translator)) {
        continue;
      }
      if (assnsWithType<recob::Hit, recob::Slice>(product, pd, ds, h_e,
                                                  translator)) {
        continue;
      }
      if (assnsWithType<recob::Edge, recob::PFParticle>(product, pd, ds, h_e,
                                                        translator)) {
        continue;
      }
      if (assnsWithType<recob::Edge, recob::SpacePoint>(product, pd, ds, h_e,
                                                        translator)) {
        continue;
      }
      if (assnsWithType<recob::PFParticle, recob::Seed>(product, pd, ds, h_e,
                                                        translator)) {
        continue;
      }
      if (assnsWithType<recob::PFParticle, recob::Slice>(product, pd, ds, h_e,
                                                         translator)) {
        continue;
      }
      if (assnsWithType<recob::PFParticle, recob::Vertex>(product, pd, ds, h_e,
                                                          translator)) {
        continue;
      }
      ...
      if (assnsWithType<recob::PFParticle, recob::Cluster>(product, pd, ds, h_e,
                                                           translator)) {
        continue;
      }
      if (assnsWithType<recob::PCAxis, recob::PFParticle>(product, pd, ds, h_e,
                                                          translator)) {
        continue;
      }
    }
...
void HepnosSPOutput::run_hepnos_func(std::function<void(void)> work) {
...
void HepnosSPOutput::beginRun(RunPrincipal const &r) {
  {
    std::function<void(void)> f = [&]() {
...
void HepnosSPOutput::beginSubRun(SubRunPrincipal const &sr) {
  {
    std::function<void(void)> f = [&]() {
...
void HepnosSPOutput::respondToOpenInputFile(FileBlock const &fb) {
  wantForwardProducts_ = fb.fileFormatVersion().era_ != "HepnosSource";
}

void HepnosSPOutput::write(EventPrincipal &p) {
  if (!p.size())
    return;
  ...
  auto map2 = this->update_map_aevent(p, h_e, batch_, wantForwardProducts_,
                                      dropCommands_);
