  ...
  , datastore_{art::ServiceHandle<icaruswf::HepnosDataStore>()->getStore()}
{
...

  {
    std::function<void(void)> f = [&]() {
      dataset_ =
        datastore_.root().createDataSet(ps().omConfig().fileName());
      return;
    };
    this->run_hepnos_func(f);
  }
...
void
HepnosSPOutput::run_hepnos_func(std::function<void(void)> work){
...
HepnosSPOutput::write(EventPrincipal& p)
{
  ...
  auto begin = std::chrono::high_resolution_clock::now();
  auto map1 = this->update_map_hevent(h_e, art::Globals::instance()->processName());
  ...
  auto map2 = this->update_map_aevent(p, h_e, batch_, wantForwardProducts_, dropCommands_);
